...
// CODE HERE
let newNums = numbers.map((num) => num / 2) 
// console.log(newNums)
...
// CODE HERE
// let newCities = cities.filter((towns) => towns.charAt( towns.length - 1) === "i")
...
// CODE HERE
let findColor = fruits.forEach((colors) => {
  if(colors.color = "red") {
    return console.log(`The fruit with index ${fruits.indexOf(colors)} is an apple.`)
  } else if (colors.color = "orange") {
    return console.log(`The fruit with index${fruits.indexOf(colors)} is an apple`)
  } else {
    console.log(`The fruit with the index of ${fruits.indexOf(colors)} in neither an apple or an orange`)
  }
})
